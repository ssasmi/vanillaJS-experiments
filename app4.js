...
form.addEventListener("submit", function (e) {
  ...
  const randomLorem = Math.floor(Math.random() * textLorem.length);
  console.log(value);
  ...
  if (isNaN(value) || 9 < value < 0) {
    result.innerHTML = `<p>${textLorem[randomLorem]}</p>`;
  } else {
    let tempText = textLorem.slice(0, value);
    tempText = tempText
      .map((text) => {
